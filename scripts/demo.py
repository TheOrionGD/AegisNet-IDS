...
# Ensure root-level scripts can import config_loader from back-end and ml_services from repo root.
...
def generate_sample_snort_logs(output_dir: Path) -> None:
    ...
    # Use ThreatSimulator for testing alerts (anomalies)
    tse = ThreatSimulator(target_ip='10.0.0.1', mode='synthetic', output_dir=output_dir)
    # Clear test file if it exists (threat_simulator appends)
    if test_file.exists():
        test_file.unlink()
        
    # Generate all attack types for the demo
    tse.run_recon('port_scan')
    tse.run_dos('syn_flood')
    tse.run_exfiltration(num_packets=20)
    tse.run_c2_beaconing(duration_mins=2, interval_secs=30)
    tse.run_web_attack('sqli')
    
    # Rename the generated log file to test_alerts.json for the demo
    # The simulator creates multiple files, we combine or pick one
    # For simplicity in demo, we'll combine all *.json in output_dir into test_alerts.json
    with test_file.open('w', encoding='utf-8') as outfile:
        # First add normal traffic
        for alert in alerts[:150]:
            outfile.write(json.dumps(alert) + '\n')
        # Then add simulator findings
        for p in output_dir.glob('*_anomalies.json'):
            with p.open('r') as infile:
                outfile.write(infile.read())
            p.unlink() # Cleanup
            
    logger.info(f'Generated comprehensive testing alerts to {test_file} using ThreatSimulator')
...
def main():
    ...
    try:
        ...
        # Always run generator to catch behavior/signatures not caught by ML
