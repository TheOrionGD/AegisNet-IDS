...
class MLEngine:
    ...
    def __init__(self, config_path: str = 'config/config.yaml'):
        ...
        self.engineer = FeatureEngineer(window_size='1min')
        self.initialized = False
        self._load_model()

    def _load_model(self):
        try:
            model_path = self.config['paths'].get('model_path', 'models/anomaly_model.joblib')
            self.model.load_model(model_path)
            self.initialized = True
            logger.info("ML Engine successfully initialized with pre-trained model.")
        except Exception as e:
            logger.warning(f"ML Engine could not load pre-trained model: {e}. Running in zero-shot/disabled mode.")
