...
class EventBus:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        if not redis_url:
            try:
                config = load_config()
                redis_url = config.get('bus', {}).get('redis_url', "redis://localhost:6379")
            except Exception:
    ...
    async def publish(self, event_type: str, data: Dict[str, Any]):
        ...
        event = {"type": event_type, "data": json.dumps(data), "ts": datetime.datetime.now(timezone.utc).isoformat()}
        ...
        if self.mode == "STREAM" and self.redis_client:
            ...
            if self.local_queue.full():
                try: self.local_queue.get_nowait()
    ...
    async def consume(self, group: Optional[str] = None):
        ...
        if self.mode == "STREAM" and self.redis_client:
            ...
            logger.info(f"Event Bus [STREAM]: Consumer {self.consumer_name} started in group {self.group_name}")
            while self.running:
                try:
