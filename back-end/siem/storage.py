...
from api.models.database import (
    Feedback, ModelVersion, ResponseAction, RuleScore,
    Incident as PGIncident, SecurityEvent as PGEvent
)
...
class _ESStub:
    ...
    def _key(self, index, doc_id): return f"{index}:{doc_id}"
    ...
    def search(self, index="*", query=None, size=100, sort=None, **kw):
        ...
        for k, docs in self._lists.items():
            ...
            if pat in k or index == "*":
                hits.extend(docs)
        hits = hits[-size:]
        return {"hits": {"hits": [{"_source": d} for d in reversed(hits)],
...
class SIEMStorage:
    ...
    def __init__(self, es_url: str = None, pg_url: str = None):
        ...
        # ── Elasticsearch / Stub ──────────────────────────────────────────────
        es_url = es_url or self.config.get('elasticsearch', {}).get('url', 'http://localhost:9200')
        ...
        if _ES_AVAILABLE:
            try:
                ...
                logger.warning(f"[STORAGE] Elasticsearch unreachable at {es_url} ({type(e).__name__}: {e}). Using in-memory stub.")
                self.es = _ESStub()
                self._es_mode = "stub"
        else:
            self.es = _ESStub()
        ...
        # ── PostgreSQL / SQLite ───────────────────────────────────────────────
        pg_url = pg_url or self.config.get('database', {}).get('url', 'sqlite:///data/siem.db')
    ...
    def _ensure_indices(self):
        ...
        try:
            if not self.es.indices.exists(index=self.incident_index):
                self.es.indices.create(index=self.incident_index, body={
                    "mappings": {
                        "properties": {
        ...
        if self._pg_mode == "live":
            try:
                ...
                if self.pg_url.startswith('postgresql'):
                    logger.warning("[STORAGE] PostgreSQL unavailable. Falling back to local SQLite.")
    ...
    def ingest_log(self, log_entry: Dict[str, Any]) -> str:
        ...
        # Schema Normalization & Null Safety
        ...
        index_name = f"cns-alerts-{datetime.datetime.now(timezone.utc).strftime('%Y.%m.%d')}"
        try:
    ...
    def store_incident(self, incident: Dict[str, Any]):
        ...
        self.es.index(index=self.incident_index, id=incident['incident_id'], document=incident)
        ...
        if self._es_mode == "stub" and self._pg_mode == "live":
            try:
                with self.engine.begin() as conn:
                    conn.execute(text("INSERT OR REPLACE INTO technical_incidents (incident_id, data, start_time) VALUES (:id, :data, :ts)"),
                                 {"id": incident['incident_id'], "data": json.dumps(incident), "ts": incident.get('start_time')})
    ...
    def get_all_incidents(self, limit: int = 200) -> List[Dict[str, Any]]:
        ...
        if self._es_mode == "live":
            res = self.es.search(index=self.incident_index, query={"match_all": {}}, size=limit, sort=[{"start_time": "desc"}])
            return [hit['_source'] for hit in res['hits']['hits']]
        elif self._pg_mode == "live":
            # Fallback to technical_incidents table
            try:
                with self.engine.connect() as conn:
                    res = conn.execute(text("SELECT data FROM technical_incidents ORDER BY start_time DESC LIMIT :limit"), {"limit": limit})
        ...
        res = self.es.search(index=self.incident_index, query={"match_all": {}}, size=limit)
        return [hit['_source'] for hit in res['hits']['hits']]
    ...
    def submit_feedback(self, incident_id: str, label: str, analyst: str = "system", notes: str = ""):
    ...
    def update_model_version(self, version_data: Dict[str, Any]):
        ...
        if not self.SessionLocal:
            ...
            return
        with self.SessionLocal() as session:
            mv = session.query(ModelVersion).filter_by(version=version_data['version']).first()
            if mv:
