...
class RuleGenerator:
    ...
    def analyze_anomalies(self, alerts_df: pd.DataFrame, anomaly_windows: List[Dict]) -> List[str]:
        ...
        """
        if alerts_df.empty or not anomaly_windows:
            logger.warning('No anomalies to analyze')
            return []
        ...
        for pattern_type, pattern_list in patterns.items():
            for pattern in pattern_list:
                if pattern.get('confidence', 0) >= 0.75:  # Confidence threshold
                    rule = self._create_rule_with_validation(pattern_type, pattern)
                    if rule:
                        generated_rules.append(rule)
                        self._audit_log(
                            f'Generated {pattern_type} rule: {rule[:80]}...',
                            'RULE_GENERATED',
                            pattern
                        )
                else:
                    self._audit_log(
                        f'Low confidence {pattern_type} pattern: {pattern.get("confidence", 0):.2f}',
                        'LOW_CONFIDENCE'
                    )
    ...
    def _detect_port_scan_pattern(self, df: pd.DataFrame, anomalies: List[Dict]) -> List[Dict]:
        ...
        for src_ip, port_count, evidence in suspicious_sources:
            ...
            if port_count < 20:  # Threshold: scanning 20+ ports is suspicious
                self._audit_log(f'Port scan src {src_ip}: only {port_count} ports (threshold 20)', 'BELOW_THRESHOLD')
                continue
    ...
    def _detect_dos_pattern(self, df: pd.DataFrame, anomalies: List[Dict]) -> List[Dict]:
        ...
        for src_ip, count in src_counts.head(5).items():
            # VALIDATION: DoS must be sustained (>100 packets in window)
            if count < 100:
                self._audit_log(f'DoS src {src_ip}: {count} packets (threshold 100)', 'BELOW_THRESHOLD')
                continue
    ...
    def _detect_c2_pattern(self, df: pd.DataFrame, anomalies: List[Dict]) -> List[Dict]:
        ...
        for dst_ip, frequency, regularity in beaconing_ips:
            ...
            if regularity < 0.7:  # Low regularity = probably not C2
                self._audit_log(f'C2 dst {dst_ip}: low regularity {regularity:.2f} (threshold 0.7)', 'LOW_REGULARITY')
                continue
    ...
    def _detect_http_anomaly_pattern(self, df: pd.DataFrame, anomalies: List[Dict]) -> List[Dict]:
        ...
        if not http_alerts.empty:
            ...
            for src_ip, count in src_http.head(3).items():
                if count >= 100:  # Threshold for HTTP flood
                    pattern = {
                        'type': 'http_flood',
                        'src_ip': src_ip,
    ...
    def save_rules(self, rules: List[str], append: bool = False) -> None:
        ...
        try:
            ...
            with self.output_path.open(mode, encoding='utf-8') as f:
                f.write(f'\n# Generated {datetime.now().isoformat()} - {len(rules)} rules\n')
                for rule in rules:
                    f.write(rule + '\n')
    ...
    def register_rule(self, sid: int, rule_text: str) -> None:
        ...
        try:
            ...
            cur.execute(
                ...
                VALUES (?, ?, 0, 0.5, ?, 0)
                ''',
                (sid, rule_text, datetime.utcnow().isoformat())
            )
            conn.commit()
            conn.close()
            ...
            if row:
                ...
                if created_at:
                    try:
                        age_days = max(
                            1,
                            (datetime.utcnow() - datetime.fromisoformat(created_at)).days
                        )
