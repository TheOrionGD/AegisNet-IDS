...
class SystemOrchestrator:
    def __init__(self):
        ...
        self.api_port = int(os.environ.get("API_PORT", 2346))
    ...
    def restart_process(self, name):
        ...
        if name == "SIEM API":
            if self.is_port_in_use(self.api_port):
                logger.info(
                    f"Port {self.api_port} still in use, waiting before restarting API..."
                )
                if not self.wait_for_port_free(self.api_port, timeout=10, delay=1):
                    logger.warning(
                        f"Port {self.api_port} still busy after waiting; attempting restart anyway."
                    )
            command = [
                self.python_exe,
                "-m",
    ...
    def boot(self):
        ...
        # Port lock: ensure configured API port is available
        if self.is_port_in_use(self.api_port):
            logger.warning(
                f"Port {self.api_port} is busy. Waiting for it to become free..."
            )
            if not self.wait_for_port_free(self.api_port, timeout=10, delay=1):
                logger.error(
                    f"Port {self.api_port} remains busy after waiting. Aborting boot."
                )
                sys.exit(1)
        ...
        if self.is_port_in_use(self.api_port):
            logger.info(
                f"Port {self.api_port} still in use, waiting before starting API..."
            )
            if not self.wait_for_port_free(self.api_port, timeout=10, delay=1):
                logger.warning(
                    f"Port {self.api_port} still busy after waiting; attempting startup anyway."
                )
        self.start_process(
            "SIEM API",
            [
