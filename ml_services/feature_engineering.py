...
class FeatureEngineer:
    ...
    def __init__(self, window_size: str = '1min'):
        ...
        self.columns = [
            'pkt_len_count', 'pkt_len_mean', 'pkt_len_max', 'pkt_len_min', 'pkt_len_std',
            'src_port_nunique', 'dst_port_nunique', 'src_ip_nunique', 'dst_ip_nunique',
            'protocol_TCP_count', 'protocol_UDP_count', 'protocol_ICMP_count',
            'packet_rate', 'mean_time_diff', 'internal_packets', 'internal_ratio',
            'dst_ip_entropy', 'packet_rate_lag_1', 'packet_rate_change'
        ]
    ...
    def normalize_features(self, features: pd.DataFrame, fit: bool = True) -> Tuple[pd.DataFrame, StandardScaler]:
        ...
        if fit:
            ...
            if not hasattr(self.scaler, 'mean_') or self.scaler.mean_ is None:
                logger.warning("StandardScaler not fitted and fit=False. Returning raw features.")
                return features, self.scaler
